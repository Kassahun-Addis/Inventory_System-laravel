...
<h2 style="text-align: center; padding:10px;">Customer List</h2>
   ...
   <div class="row mb-3" style="display: flex; justify-content: space-between; align-items: center;">
    ...
    <div class="col-12 col-md-6 d-flex justify-content-between mb-2 mb-md-0">
        <!-- Per Page Selection -->
        <form action="{{ route('customers.index') }}" method="GET" class="form-inline" style="flex: 1;">
            ...
            <div class="form-group w-100" style="display: flex; align-items: center;">
                ...
                <div class="d-block d-md-none dropdown ml-1">
                    ...
                    <div class="dropdown-menu" aria-labelledby="exportDropdown">
                        ...
                        <a class="dropdown-item" href="{{ route('customers.export') }}">Excel</a>
                    </div>
                </div>
                ...
                <div class="d-none d-md-block ml-1">
                    ...
                    <button type="button" class="btn btn-primary ml-1" onclick="window.location.href='{{ route('customers.export') }}'">Excel</button>
                </div>
            </div>
        </form>
    </div>
...
<div class="mt-3 d-flex justify-content-between align-items-center">
    <div>
        ...
        @else
            <a href="{{ $customers->previousPageUrl() }}" class="btn btn-light">Previous</a>
        @endif
        ...
        @foreach (range(1, $customers->lastPage()) as $i)
            ...
            @else
                <a href="{{ $customers->url($i) }}" class="btn btn-light">{{ $i }}</a>
            @endif
        @endforeach

        @if ($customers->hasMorePages())
            <a href="{{ $customers->nextPageUrl() }}" class="btn btn-light">Next</a>
        @else
...
const customers = @json($customers->items()); // Use items() to get the array of bank details

// Check if customers is an array
console.log(customers); // Debugging line to see the content of $customers

const printWindow = window.open('', '', 'height=500,width=800');
printWindow.document.write('<html><head><title>All customers Details</title>');
printWindow.document.write('<style>table { width: 100%; border-collapse: collapse; }');
printWindow.document.write('th, td { border: 1px solid black; padding: 10px; text-align: left; }</style>');
printWindow.document.write('</head><body>');
printWindow.document.write('<h2>All customers Details</h2>');
printWindow.document.write('<table>');
