...
@section('content')
<h2 style="text-align: center; padding:10px;">Assets</h2>
<div class="container mt-5">
<div class="d-flex justify-content-between align-items-center mb-3">
    <!-- <a href="{{ route('assets.create') }}" class="btn btn-primary">Add New</a> -->
    </div>
   ...
   <div class="row mb-3" style="display: flex; justify-content: space-between; align-items: center;">
    ...
    <div class="col-12 col-md-6 d-flex justify-content-between mb-2 mb-md-0">
        <!-- Per Page Selection -->
        <form action="{{ route('assets.index') }}" method="GET" class="form-inline" style="flex: 1;">
        ...
        <!-- Add New Button -->
        <a href="{{ route('assets.create') }}" class="btn btn-primary ml-2">Add New</a>
    </div>
            ...
            <div class="form-group w-100" style="display: flex; align-items: center;">
                ...
                <div class="d-block d-md-none dropdown ml-1">
                    ...
                    <div class="dropdown-menu" aria-labelledby="exportDropdown">
                        ...
                        <a class="dropdown-item" href="{{ route('assets.export') }}">Excel</a>
                    </div>
                </div>
                ...
                <div class="d-none d-md-block ml-1">
                    ...
                    <button type="button" class="btn btn-primary ml-1" onclick="window.location.href='{{ route('assets.export') }}'">Excel</button>
                </div>
            </div>
        </form>
    </div>
...
<div class="mt-3">
    ...
    <div class="mt-3 d-flex justify-content-between align-items-center">
        <div>
            ...
            @else
                <a href="{{ $assets->previousPageUrl() }}" class="btn btn-light">Previous</a>
            @endif

            @foreach (range(1, $assets->lastPage()) as $i)
                @if ($i == $assets->currentPage())
                    <span class="btn btn-primary disabled">{{ $i }}</span>
                @else
                    <a href="{{ $assets->url($i) }}" class="btn btn-light">{{ $i }}</a>
                @endif
            @endforeach

            @if ($assets->hasMorePages())
                <a href="{{ $assets->nextPageUrl() }}" class="btn btn-light">Next</a>
            @else
...
function printAllBankDetails() {
    const assets = @json($assets->items()); // Use items() to get the array of bank details

    // Check if assets is an array
    console.log(assets); // Debugging line to see the content of $assets

    const printWindow = window.open('', '', 'height=500,width=800');
    printWindow.document.write('<html><head><title>All assets Details</title>');
    printWindow.document.write('<style>table { width: 100%; border-collapse: collapse; }');
    printWindow.document.write('th, td { border: 1px solid black; padding: 10px; text-align: left; }</style>');
    printWindow.document.write('</head><body>');
    printWindow.document.write('<h2>All assets Details</h2>');
    printWindow.document.write('<table>');
